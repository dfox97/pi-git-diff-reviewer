...
async function getMergeBase(pi: ExtensionAPI, repoRoot: string, baseBranch: string): Promise<string | null> {
  const result = await pi.exec("git", ["merge-base", baseBranch, "HEAD"], { cwd: repoRoot });
  if (result.code !== 0) {
    return null;
  }
...
async function resolveBranch(pi: ExtensionAPI, repoRoot: string, branch: string): Promise<string | null> {
  const result = await pi.exec("git", ["rev-parse", "--verify", branch], { cwd: repoRoot });
  if (result.code !== 0) {
    return null;
  }
  const sha = result.stdout.trim();
...
export async function getReviewWindowData(pi: ExtensionAPI, cwd: string, baseBranch?: string): Promise<{ repoRoot: string; files: ReviewFile[]; commits: { sha: string; shortSha: string; subject: string }[]; baseBranch?: string; mergeBase?: string }> {
  const repoRoot = await getRepoRoot(pi, cwd);
  const repositoryHasHead = await hasHead(pi, repoRoot);

  let mergeBase: string | undefined;
  let baseBranchSha: string | undefined;
  if (baseBranch && repositoryHasHead) {
    const resolved = await resolveBranch(pi, repoRoot, baseBranch);
    if (resolved == null) {
      throw new Error(`Base branch "${baseBranch}" not found.`);
    }
    baseBranchSha = resolved;
    const base = await getMergeBase(pi, repoRoot, baseBranch);
    if (base == null) {
  ...
  const trackedDiffOutput = repositoryHasHead
    ? mergeBase != null
      ? await runGit(pi, repoRoot, ["diff", "--find-renames", "-M", "--name-status", `${mergeBase}..HEAD`, "--"])
      : await runGit(pi, repoRoot, ["diff", "--find-renames", "-M", "--name-status", "HEAD", "--"])
    : "";
  const untrackedOutput = mergeBase != null ? "" : await runGitAllowFailure(pi, repoRoot, ["ls-files", "--others", "--exclude-standard"]);
  const trackedFilesOutput = await runGitAllowFailure(pi, repoRoot, ["ls-files", "--cached"]);
  const deletedFilesOutput = mergeBase != null ? "" : await runGitAllowFailure(pi, repoRoot, ["ls-files", "--deleted"]);
  ...
  const commits = repositoryHasHead
    ? parseCommitLog(
        mergeBase != null
          ? await runGitAllowFailure(pi, repoRoot, ["log", `${mergeBase}..HEAD`, "--max-count=50", "--format=%H%x09%h%x09%s"])
          : await runGitAllowFailure(pi, repoRoot, ["log", "--max-count=50", "--format=%H%x09%h%x09%s"]),
      )
    : [];
...
export async function loadReviewFileContents(pi: ExtensionAPI, repoRoot: string, file: ReviewFile, scope: ReviewScope, commitSha?: string, mergeBase?: string): Promise<ReviewFileContents> {
  if (scope === "all-files") {
