...
import { open, type GlimpseWindow } from "./wsl-glimpse.js";
...
export default function (pi: ExtensionAPI) {
  ...
  async function reviewRepository(ctx: ExtensionCommandContext, baseBranch?: string): Promise<void> {
    if (activeWindow != null) {
      ctx.ui.notify("A review window is already open.", "warning");
      return;
    }

    const { repoRoot, files, commits, mergeBase } = await getReviewWindowData(pi, ctx.cwd, baseBranch);
    if (files.length === 0) {
      ctx.ui.notify("No reviewable files found.", "info");
      return;
    }

    const html = buildReviewHtml({ repoRoot, files, commits, baseBranch, mergeBase });
    ...
    const loadContents = (file: ReviewFile, scope: ReviewRequestFilePayload["scope"], commitSha?: string): Promise<ReviewFileContents> => {
      ...
      const pending = loadReviewFileContents(pi, repoRoot, file, scope, commitSha, mergeBase);
